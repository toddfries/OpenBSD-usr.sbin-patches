...
runner_timeout(int fd, short event, void *p)
{
	 ...
	 */
	if (! rq_off_done)
		rq_off_done = ramqueue_load_offline(&env->sc_rqueue);
	...
	while (rq_evp) {
		...
		/* current envelope doesn't want to dance, grab her friend */
	...
	if (rq_done && rq_off_done && TAILQ_FIRST(&env->sc_rqueue.queue) == NULL) {
